...
def evaluate(images_dir: Path = DEFAULT_OUT) -> dict:
    ...
    (RESULTS_DIR / "samples").mkdir(parents=True, exist_ok=True)
    ...
    for rec in records:
        ...
        if is_tampered:
            ...
            pt = per_type.setdefault(ttype, {"n": 0, "hits": 0, "iou_sum": 0.0})
            pt["n"] += 1
            ...
            pt["hits"] += int(hit)
            pt["iou_sum"] += _iou(pred, gt)

            # save up to 2 annotated overlays per tamper type for the deck
            if res.get("annotated_b64") and samples_saved.get(ttype, 0) < 2:
                samples_saved[ttype] = samples_saved.get(ttype, 0) + 1
                (RESULTS_DIR / "samples" / f"{rec['id']}.png").write_bytes(
                    base64.b64decode(res["annotated_b64"]))
    ...
    metrics = {
        "dataset": {"n_clean": data["n_clean"], "n_tampered": data["n_tampered"],
...
def _write_results(m: dict) -> None:
    ...
    for t, v in m["localization"]["by_tamper_type"].items():
        lines.append(f"| {t} | {v['n']} | {v['localization_hit_rate']} | {v['mean_iou']} |")
    lines += [
        "",
        "Note: copy-move (clone) detection on dense document text is corroboration-only here "
        "(repeated glyphs/amounts make standalone clone detection unreliable); it is the job of "
        "the learned DocTamper model (Day 3). The noise-loss + ELA detectors catch the realistic "
        "paint/splice/recompress edits with zero false positives on clean documents.",
        "",
...
def main() -> int:
    ...
    print(f"Localization: hit_rate={m['localization']['hit_rate']} "
          f"mean_iou={m['localization']['mean_iou']}")
    for t, v in m["localization"]["by_tamper_type"].items():
        print(f"  - {t:12s} n={v['n']} hit={v['localization_hit_rate']} iou={v['mean_iou']}")
